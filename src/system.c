...
void createNewAccount(struct User u)
{
    ...
    {   
        ...
        {
            printf("***Can't be created. This account already exists for this user***\n\n");
            fclose(pf);
            goto noAccount;
        }
    }
    ...
    if (u.id == -1) 
    {
        printf("Error by receiving user ID\n\n");
    }
...
account:;
    ...
    printf("\nPlease write account number that will be changed: ");
    scanf("%d", &r.accountNbr);
...
void transferOwner(struct User u)
{
    ...
    printf("\nPlease enter the account number you want to transfer ownership: ");
    scanf("%d",&r.accountNbr);
...
void  makeTransaction(struct User u)
{
    ...
    if (!exist) {
        printf("Account was not found");
         system("clear");
        printf("\nThere is no such account in database");
    ...
    {   
        printf("\nYou can't deposit or withdraw cash in fixed (fixed01, fixed02, fixed03) accounts!\n\n");
        sleep(2);
        mainMenu(u);
    }
    ...
    else if (option == 2)
    {
        ...
        if(cr.amount < money){
            ...
            printf("\nThe amount you have chosen to withdraw exceeds your available balance.\nTry again ...");
